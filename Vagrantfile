Vagrant.configure("2") do |config|
  ...
  config.vm.define "velgard" do |velgard|
    ...
    velgard.vm.provision :shell, privileged: false, path: "scripts/config-nodejs-env.sh"
    velgard.vm.provision :shell, privileged: false, path: "scripts/config-python-env.sh"
    velgard.vm.provision :shell, privileged: false, path: "scripts/config-ruby-env.sh"
